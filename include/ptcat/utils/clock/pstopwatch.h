...
namespace ptcat {
    namespace clock {
        ...
        private:
            bool is_running_;
            std::chrono::microseconds time_;//elapsed time
            std::chrono::steady_clock::time_point time_points_;
        public:
            explicit LapInfo() : is_running_{true}, time_{std::chrono::microseconds(0)}, time_points_(std::chrono::steady_clock::now()){}
            explicit LapInfo(bool is_running, std::chrono::microseconds time) : is_running_{is_running}, time_{time}, time_points_(std::chrono::steady_clock::now()){}
            explicit LapInfo(bool is_running, std::chrono::microseconds time, std::chrono::steady_clock::time_point time_points) : is_running_{is_running}, time_{time}, time_points_(time_points){}

            const std::chrono::steady_clock::time_point& GetTimePoints() {
                return this->time_points_;
            }

            void Reset() {
                is_running_ = true;
                time_points_ = std::chrono::steady_clock::now();
            }

            bool IsRunning(){return is_running_;}

            void AddTime(std::chrono::microseconds new_time) {
                time_ += new_time;
            }
        ...
        public:
            ...
            llong MicroLapTime(const std::string& name);

            llong MilliLapTime(const std::string& name);

            llong LapTime(const std::string& name);
        };
    }
