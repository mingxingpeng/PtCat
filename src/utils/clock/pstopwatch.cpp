    ...
    namespace clock {
        ...
        void StopWatch::LapStop(const std::string& name) {
            ...
            if(it->second.IsRunning()) {
                ...
                it->second.AddTime(std::chrono::duration_cast<std::chrono::microseconds>(end_time_ - it->second.GetTimePoints()));
            }
        }
        ...
        llong StopWatch::MicroLapTime(const std::string& name) {
        ...
        llong StopWatch::MilliLapTime(const std::string& name){
            std::shared_lock<std::shared_mutex> lock(s_mux_);
            auto it = laps_.find(name);
            if (it == laps_.end())
                return 0;
        ...
        llong StopWatch::LapTime(const std::string& name){
            std::shared_lock<std::shared_mutex> lock(s_mux_);
            auto it = laps_.find(name);
            if (it == laps_.end())
                return 0;
