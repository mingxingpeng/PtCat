...
#include "ptcat/utils/clock/pchronograph.h"
#include <cassert>
...
namespace ptcat {
    namespace clock {
        ...
        void ChronoGraph::Start(){
            ...
            elapsed_time_ = std::chrono::microseconds(0);
            start_time_ = std::chrono::steady_clock::now();
            is_running_.store(true);//indicates that it is runing
        }

        //stop record
        llong ChronoGraph::Stop() {
            std::unique_lock<std::shared_mutex> lock(s_mux_);
            //判断程序有没有开启监测
            ...
            elapsed_time_ = std::chrono::duration_cast<std::chrono::microseconds>(end_time_ - start_time_);
